...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <br>
        <div>
            @foreach($orderProduct as $orderProductNumber)
                <div>
                    <ul>
                        <li>{{$orderProductNumber->product_id}}</li>
                        <li>{{$orderProductNumber->quantity}}</li>
                        <form action="{{route('shipping')}}" method="POST" id="shipping-form">
                        {{csrf_field()}}
                            <input type="hidden" class="form-control" id="shipping" name="shipping" value={{$productNo}}>
