            ...
            <div class="sidebar">
                ...
                <ul>
                    @foreach ($categories as $category)
                        <li><a href="{{route('layout.shop',['category' => $category->slug])}}">{{$category->name}}</a></li>
                    @endforeach
                </ul>
            </div>
            <div>
                ...
                {{$products->appends(['page'=> $products->currentPage(), 'category'=> $categoryName])->render('pagination::bootstrap-4')}}
            </div>
        </div>
        <footer>
